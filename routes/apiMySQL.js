module.exports = function(app,connection,csvParse,fs,moment,pool) {
  ...
  app.get('/api/mySQL/exportCSVtest', function(req, res) {
    ...
    var filePath = "Primrose Solar Limited_131015.csv";
      ...
      csvParse(csvData, {
        ...
        if (err) {
          ...
          var readingsForExport = mpanList.map(function (mpan) {
            ...
            for (j=0;j<readingsForExport[4].data.length;j++){ // use this line only for histroical data
              var day = moment(readingsForExport[4].data[j][2], "DD/MM/YYYY").format("YYYY-MM-DD");
              var hour = moment("00:00", "HH:mm").format("HH:mm");
              for (i = 3; i < readingsForExport[4].data[j].length; i++) {

                if (readingsForExport[4].data[j][i] === "-"){
                  readingsForExport[4].data[j][i] = "NULL";
                }

                sqlInputData[n] = ["(NULL, '" + day + "','" + hour + "'," + readingsForExport[4].data[j].length+")"];
                hour = moment(hour,"HH:mm").add(30,'minutes').format("HH:mm");
                n++;
              }
            }
            res.send("INSERT INTO export_" + readingsForExport[4].id + " VALUES " + sqlInputData + "  ON DUPLICATE KEY UPDATE generation=VALUES(generation)");
            ...
            // });
            // res.send(readingsForExport[4].data[0][15] + "\n" + readingsForExport[4].data[0].length + "\n" + readingsForExport[4].data[0]);
