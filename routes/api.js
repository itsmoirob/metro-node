module.exports = function(app, connection) {
	...
	app.get('/api/reports/incidentsSite/:id', function(req, res) {
		var id = req.params.id;
		connection.query('select id, site, date_logged, start_time, end_time, reported_by_person, (select value from incident_report_company where id = reported_by_company) as reported_by_company, (select value from incident_report_category where id = category) as category,(select value from incident_report_planned where id = planned) as planned, (select value from incident_report_generation_loss where id = loss_of_generation) as loss_of_generation, details, case when status = 1 then \'Open\' else \'Closed\' end as status, incident_report_number from incident_log where site = ' + id + ' order by start_time asc;', function(err, rows) {
			if (err) {
				return res.json(err);
			} else {
				return res.json(rows);
	...
	app.get('/api/displaySite/portfolioSiteDataMonth/:month', function(req, res) {
		...
		connection.query('select pG.date, pG.ps1 as pPS1, pG.ps2 as pPS2, pG.ps3 as pPS3, pG.ps4 as pPS4, pG.ps5 as pPS5, pG.ps11 as pPS11, pG.ps12 as pPS12, pG.ps13 as pPS13, pG.ps15 as pPS15, (ifnull(pG.ps1,0) + ifnull(pG.ps2,0) + ifnull(pG.ps3,0) + ifnull(pG.ps4,0) + ifnull(pG.ps5,0) + ifnull(pG.ps11,0) + ifnull(pG.ps12,0) + ifnull(pG.ps13,0) + ifnull(pG.ps15,0)) as pPSAll, aG.ps1 as aPS1, aG.ps2 as aPS2, aG.ps3 as aPS3, aG.ps4 as aPS4, aG.ps5 as aPS5, aG.ps11 as aPS11, aG.ps12 as aPS12, aG.ps13 as aPS13, aG.ps15 as aPS15, (ifnull(aG.ps1,0) + ifnull(aG.ps2,0) + ifnull(aG.ps3,0) + ifnull(aG.ps4,0) + ifnull(aG.ps5,0) + ifnull(aG.ps11,0) + ifnull(aG.ps12,0) + ifnull(aG.ps13,0) + ifnull(aG.ps15,0)) as aPSAll, pG.ps1 / (pE.ps1 * (select tic_mwp * 1000 from top_table where id = 1)) as pPR1, pG.ps2 / (pE.ps2 * (select tic_mwp * 1000 from top_table where id = 2)) as pPR2, pG.ps3 / (pE.ps3 * (select tic_mwp * 1000 from top_table where id = 3)) as pPR3, pG.ps4 / (pE.ps4 * (select tic_mwp * 1000 from top_table where id = 4)) as pPR4, pG.ps5 / (pE.ps5 * (select tic_mwp * 1000 from top_table where id = 5)) as pPR5, pG.ps11 / (pE.ps11 * (select tic_mwp * 1000 from top_table where id = 11)) as pPR11, pG.ps12 / (pE.ps12 * (select tic_mwp * 1000 from top_table where id = 12)) as pPR12, pG.ps13 / (pE.ps13 * (select tic_mwp * 1000 from top_table where id = 13)) as pPR13, pG.ps15 / (pE.ps15 * (select tic_mwp * 1000 from top_table where id = 15)) as pPR15,(pG.ps1 + pG.ps2 + pG.ps3 + pG.ps4 + pG.ps5 + pG.ps11 + pG.ps12 + pG.ps13 + pG.ps15 ) / ((pE.ps1 * (select tic_mwp * 1000 from top_table where id = 1)) + (pE.ps2 * (select tic_mwp * 1000 from top_table where id = 2)) + (pE.ps3 * (select tic_mwp * 1000 from top_table where id = 3)) + (pE.ps4 * (select tic_mwp * 1000 from top_table where id = 4)) + (pE.ps5 * (select tic_mwp * 1000 from top_table where id = 5)) + (pE.ps11 * (select tic_mwp * 1000 from top_table where id = 11)) + (pE.ps12 * (select tic_mwp * 1000 from top_table where id = 12)) + (pE.ps13 * (select tic_mwp * 1000 from top_table where id = 13)) + (pE.ps15 * (select tic_mwp * 1000 from top_table where id = 15))) as pPRAll, aG.ps1 / (aE.ps1 * (select tic_mwp * 1000 from top_table where id = 1)) as aPR1, aG.ps2 / (aE.ps2 * (select tic_mwp * 1000 from top_table where id = 2)) as aPR2, aG.ps3 / (aE.ps3 * (select tic_mwp * 1000 from top_table where id = 3)) as aPR3, aG.ps4 / (aE.ps4 * (select tic_mwp * 1000 from top_table where id = 4)) as aPR4, aG.ps5 / (aE.ps5 * (select tic_mwp * 1000 from top_table where id = 5)) as aPR5, aG.ps11 / (aE.ps11 * (select tic_mwp * 1000 from top_table where id = 11)) as aPR11, aG.ps12 / (aE.ps12 * (select tic_mwp * 1000 from top_table where id = 12)) as aPR12, aG.ps13 / (aE.ps13 * (select tic_mwp * 1000 from top_table where id = 13)) as aPR13, aG.ps15 / (aE.ps15 * (select tic_mwp * 1000 from top_table where id = 15)) as aPR15, (aG.ps1 + aG.ps2 + aG.ps3 + aG.ps4 + aG.ps5 + aG.ps11 + aG.ps12 + aG.ps13 + aG.ps15 ) / ((aE.ps1 * (select tic_mwp * 1000 from top_table where id = 1)) + (aE.ps2 * (select tic_mwp * 1000 from top_table where id = 2)) + (aE.ps3 * (select tic_mwp * 1000 from top_table where id = 3)) + (aE.ps4 * (select tic_mwp * 1000 from top_table where id = 4)) + (aE.ps5 * (select tic_mwp * 1000 from top_table where id = 5)) + (aE.ps11 * (select tic_mwp * 1000 from top_table where id = 11)) + (aE.ps12 * (select tic_mwp * 1000 from top_table where id = 12)) + (aE.ps13 * (select tic_mwp * 1000 from top_table where id = 13)) + (aE.ps15 * (select tic_mwp * 1000 from top_table where id = 15))) as aPRAll from monthlyPredictedGeneration pG join monthlyPredictedEsol pE on pG.date = pE.date join (select date_format(date, "%Y-%m") as date, sum(ps1) as ps1, sum(ps2) as ps2, sum(ps3) as ps3, sum(ps4) as ps4, sum(ps5) as ps5, sum(ps11) as ps11, sum(ps12) as ps12, sum(ps13) as ps13, sum(ps15) as ps15 from dailySumExport group by date_format(date, "%Y-%m")) aG on date_format(pG.date, "%Y-%m") = aG.date join (select date_format(date, "%Y-%m") as date, sum(ps1) as ps1,  sum(ps2) as ps2, sum(ps3) as ps3, sum(ps4) as ps4, sum(ps5) as ps5, sum(ps11) as ps11, sum(ps12) as ps12, sum(ps13) as ps13, sum(ps15) as ps15 from dailyEsol group by date_format(date, "%Y-%m")) aE on date_format(pG.date, "%Y-%m") = aE.date where date_format(pG.date, "%Y-%m") = "' + month + '";', function(err, rows) {
			if (err) {
				return res.json(err);
			} else {
				return res.json(rows);
